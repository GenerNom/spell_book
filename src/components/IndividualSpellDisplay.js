...
const IndividualSpellDisplay  = ({spell}) => {
    ...
    return (
        <div className="displaySpell" onClick={() =>  toggleVisibility()}>
            <div className="header"><h3 className="spellName">{spell.name}</h3> <h5 className="spellLevel">Level:{spell.level}  Classes:{spell.class.join(", ")} </h5><div className="smallText">{isRitualSymbol}</div><div className={"toggle"} ><h3> {visibilitySymbol}</h3></div></div>
